...
# MAGIC %md
# MAGIC ## Gold Layer Pipeline Summary — Maven Market
# MAGIC
# MAGIC This notebook defines the **Gold layer** of the Maven Market lakehouse using **Lakeflow Spark Declarative Pipelines (DLT)**. It reads from Silver tables, applies business logic, and produces analytics-ready dimensions, facts, and aggregates.
# MAGIC
# MAGIC ### Configuration
# MAGIC * Loads catalog and schema names from `/Workspace/maven_market/Mysore-Pak/config/config.yml`
# MAGIC * All tables are written to the configured **gold schema**
# MAGIC
# MAGIC ### Dimension Tables
# MAGIC | Table | Source | Description |
# MAGIC | --- | --- | --- |
# MAGIC | `dim_customer` | `silver_scd_customers` | Current customer snapshot (SCD2, `__END_AT IS NULL`). Includes demographics: income, education, occupation, homeowner status |
# MAGIC | `dim_product` | `silver_scd_products` | Current product snapshot. Includes pricing, weight, recyclable/low-fat flags |
# MAGIC | `dim_store` | `silver_scd_stores` + `silver_regions` | Current store snapshot joined with region/district info |
# MAGIC
# MAGIC ### Fact Tables
# MAGIC | Table | Source | Description |
# MAGIC | --- | --- | --- |
# MAGIC | `fact_sales` | `silver_transactions_valid` + `silver_orders_valid` + `silver_scd_products` | Unions transactions and orders, joins product pricing to compute `total_revenue`, `total_cost`, and `total_profit` |
# MAGIC | `fact_returns` | `silver_returns` + `silver_scd_products` | Returns with `return_quantity` by product and store |
# MAGIC
# MAGIC > **Note:** Cells 4, 5, and 6 contain three versions of `fact_sales`. Only one should be active in a pipeline run — the others should be removed or commented out to avoid conflicts.
# MAGIC
# MAGIC ### Aggregate / KPI Tables
# MAGIC | Table | Grain | Key Metrics |
# MAGIC | --- | --- | --- |
# MAGIC | `agg_daily_sales` | `transaction_date`, `store_id` | total units, revenue, cost, profit, unique customers, avg transaction value |
# MAGIC | `agg_monthly_sales` | `year`, `month`, `store_id`, `product_brand` | total units, revenue, cost, profit, unique customers, transactions |
# MAGIC | `agg_store_performance` | `store_id` | total units sold, revenue, profit, units returned, return rate (%) |
# MAGIC | `agg_return_kpi` | `product_id` | sold qty, returned qty, return rate |
